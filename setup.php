...
<?php
  ...
  foreach ($cfg as $id => $value) {
    ...
    if ($eval) $cfg[$id] = $value; //corrected value
    else {
        ...
        while (1) {
          ...
          if (!empty($response)) {
            ...
            if ($eval) {
              ...
              break;
            } else echo "Invalid response; please answer with a " . gettype2($id) . ":\n";
          } else break;
        }
      }
    }
  ...
  if ($cleaninstall) {
    ...
    } else echo "WARNING: cannot make the users.json file if the USERS_FILE or CONFIG_PATH constants were changed from default values!";
  }
  ...
  function rrmdir($dir)
  {
    if (is_dir($dir)) {
      $files = scandir2($dir);
      foreach ($files as $file) rrmdir("$dir/$file");
      rmdir($dir);
    } else {
