...
function readAscii($lines, $coordinates = 0)
{
    ...
    foreach ($lines as $line) {
        ...
        if (($coordinates > 0) and ($j + 1 > $coordinates)) {
            break;
        }  // stop on number of coordinates
...
function readJcamp($lines)
{
    ...
    while (!isset($parameters["##XYDATA"])) {
        ...
        if (in_array($parameter, $paramList)) {
            $parameters[$parameter] = trim($value);
        }
        break;
        default:
        // $parameter from the previous line (with ## or $$) is still valid!
        if (in_array($parameter, $paramList)) {
            $parameters[$parameter] = $parameters[$parameter]." ".trim($value);
        }
        break;
    }
    }
    
    if (!isset($parameters["##XFACTOR"])) {
        $parameters["##XFACTOR"] = 1;
    }
    if (!isset($parameters["##YFACTOR"])) {
        $parameters["##YFACTOR"] = 1;
    }
    ...
    // final sanity checks
    $sanityCheck = array();
    if (isset($parameters["##FIRSTX"])) {
        if (!compareDecimals($array[0][0], $parameters["##FIRSTX"])) {
            array_push($sanityCheck, "firstx");
        }
    }
    if (isset($parameters["##FIRSTY"])) {
        if (!compareDecimals($array[0][1], $parameters["##FIRSTY"])) {
            array_push($sanityCheck, "firsty");
        }
    }
    if (isset($parameters["##LASTX"])) {
        if (!compareDecimals($array[count($array) - 1][0], $parameters["##LASTX"])) {
            array_push($sanityCheck, "lastx");
        }
    }
    if (isset($parameters["##NPOINTS"])) {
        if ((count($array) != $parameters["##NPOINTS"]) or (count($array) ==0)) {
            array_push($sanityCheck, "npoints");
        }
    }
    ...
    if (isset($parameters["##FIRSTX"])) {
        $nod_x = nod($parameters["##FIRSTX"]);
    } elseif (isset($parameters["##LASTX"])) {
        $nod_x = nod($parameters["##LASTX"]);
    } else {
        $nod_x = 3;
